...
public class TreeMap<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V>, Cloneable, Serializable {
	...
	private abstract static class AscendingSubMapIterator<K, V> extends AbstractSubMapIterator<K, V> {
		...
		final TreeMap.Entry<K, V> getStartNode() {
			if (subMap.fromStart) {
				return subMap.loInclusive ? subMap.biggerOrEqualEntry(subMap.lo) : subMap.biggerEntry(subMap.lo);
			}
		...
		BoundedMapIterator(Node<K, V> startNode, int startOffset, TreeMap<K, V> map, Node<K, V> finalNode, int finalOffset) {
			...
			if (startNode != null) {
				if (node == this.finalNode && offset >= this.finalOffset) {
					...
					Comparable<K> object = backingMap.comparator == null ? toComparable(startNode.keys[startOffset]) : null;
					if (this.backingMap.cmp(object, node.keys[offset], this.finalNode.keys[this.finalOffset]) > 0) {
						node = null;
					}
				}
			}
	...
	static class AscendingSubMapEntrySet<K, V> extends AbstractSet<Map.Entry<K, V>> implements NavigableSet<Map.Entry<K, V>> {
		...
		public NavigableSet<java.util.Map.Entry<K, V>> subSet(java.util.Map.Entry<K, V> start, boolean startInclusive, java.util.Map.Entry<K, V> end,
			...
			if (map.fromStart
					&& ((!map.loInclusive && endInclusive) ? map.m.keyCompare(end.getKey(), map.lo) <= 0 : map.m.keyCompare(end.getKey(), map.lo) < 0)) {
				throw new IllegalArgumentException();
			}
			if (map.toEnd
					&& ((!map.hiInclusive && startInclusive) ? map.m.keyCompare(start.getKey(), map.hi) >= 0 : map.m.keyCompare(start.getKey(), map.hi) > 0)) {
				throw new IllegalArgumentException();
			}
		...
		public NavigableSet<java.util.Map.Entry<K, V>> tailSet(java.util.Map.Entry<K, V> start, boolean startInclusive) {
			...
			int result;
			if (map.toEnd) {
				result = (null != comparator()) ? comparator().compare(start.getKey(), map.hi) : toComparable(start.getKey()).compareTo(map.hi);
				isInRange = (map.hiInclusive || !startInclusive) ? result <= 0 : result < 0;
			}
			if (map.fromStart) {
				result = (null != comparator()) ? comparator().compare(start.getKey(), map.lo) : toComparable(start.getKey()).compareTo(map.lo);
	...
	static class DescendingSubMapEntrySet<K, V> extends AbstractSet<Map.Entry<K, V>> implements NavigableSet<Map.Entry<K, V>> {
		NavigableSubMap<K, V> map;
		...
		public java.util.Map.Entry<K, V> ceiling(java.util.Map.Entry<K, V> e) {
			...
			if (!map.checkLowerBound(node.key)) {
				Comparable<K> object = map.comparator() == null ? toComparable(e.getKey()) : null;
		...
		public NavigableSet<java.util.Map.Entry<K, V>> descendingSet() {
			...
			if (map.fromStart) {
				return new AscendingSubMapEntrySet<K, V>(new AscendingSubMap<K, V>(map.m, map.lo, map.loInclusive));
			}
			if (map.toEnd) {
				return new AscendingSubMapEntrySet<K, V>(new AscendingSubMap<K, V>(map.hi, map.hiInclusive, map.m));
			}
			return new AscendingSubMapEntrySet<K, V>(new AscendingSubMap<K, V>(map.m));
		}
		...
		public java.util.Map.Entry<K, V> floor(java.util.Map.Entry<K, V> e) {
			...
			if (!map.checkLowerBound(node.key)) {
				Comparable<K> object = map.m.comparator == null ? toComparable(e.getKey()) : null;
		...
		public NavigableSet<java.util.Map.Entry<K, V>> subSet(java.util.Map.Entry<K, V> start, boolean startInclusive, java.util.Map.Entry<K, V> end,
			...
			if (map.fromStart
					&& ((!map.loInclusive && startInclusive) ? map.cmp(startobject, start.getKey(), map.lo) <= 0
							: map.cmp(startobject, start.getKey(), map.lo) < 0)
					|| (map.toEnd && ((!map.hiInclusive && endInclusive) ? map.cmp(endobject, end.getKey(), map.hi) >= 0 : map.cmp(endobject, end.getKey(),
							map.hi) > 0))) {
				throw new IllegalArgumentException();
			}
		...
		public NavigableSet<java.util.Map.Entry<K, V>> tailSet(java.util.Map.Entry<K, V> start, boolean startInclusive) {
			if (map.toEnd) {
				...
				return new DescendingSubMapEntrySet<K, V>(new DescendingSubMap<K, V>(start.getKey(), startInclusive, map.m));
			}
		}
		...
		int keyCompare(K left, K right) {
			return (null != map.comparator()) ? map.comparator().compare(left, right) : toComparable(left).compareTo(right);
		}
